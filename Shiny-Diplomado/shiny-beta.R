                   ...
                   box(status = "primary",
                       ...
                       HTML('<p>Este dashboard fue creado como evaluación para
                         ...
                         en la PUC.</p>
                            ...
                            los precios historicos de las acciones obtenidas
                            a traves de una función de webscrapping desde
                            la página de "Inversiones" de el diario El Mercurio.')
...
server <- function(input, output) {
  ...
  output$plot_test <- renderDygraph({
    ...
    p <- dygraph(serietiempo) %>%
      dyOptions(labelsUTC = TRUE, fillGraph=TRUE, fillAlpha=0.1, drawGrid = FALSE, colors="#1F5861") %>%
      dyRangeSelector() %>%
